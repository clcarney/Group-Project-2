...
set.seed(999)
loans_complete_skew<- loans_completed[order(runif(4000)), ]
loans_new<-rbind(loans_complete_skew,loansDefualted)
table(loans_new$default) #Now roughly 38%
```
###DATA CLEANING
...
plot(loans_new$scorex, main = "Number of Loans vs Scorex", xlab = "Scorex", ylab = "Number of Loans", cex.names = 0.7) #Most loans are only for high credit score borrowers
par(op)
...
plot(loans_new$income_range_description, main = "Number of Loans vs Income", xlab = "Income Range", ylab = "Number of Loans")#Confirmed, only loans given to higher income borrowers
par(op)
...
```{r}
#Linear Regression -- Copied From Class --- Need to Redo
...
library(lmtest)
library(caret)
loans.m1<-glm(formula = default ~ . , data = loans_new, family ='binomial')
summary(loans.m1) #Need to so some removing of conflicting variables

full<-glm(formula = default~.,data = loans_new, family = 'binomial')
null<-glm(formula = default~1,data = loans_new, family = 'binomial')
forwardSteps<-step(null,scope = list(lower = null, upper = full),data = loans_new, direction = 'forward')

loans2<-subset(loans_new,select = c(default, month, stated_monthly_income, listing_term, weekday, 
    monthly_debt, lender_indicator, total_inquiries, public_records_last12_months, 
    dti_wprosper_loan, income_verifiable))

#Split Data
set.seed(12345)
loans_rand <- loans2[order(runif(5538)), ]
loans2_train<-loans_rand[1:3800,]
loans2_test<-loans_rand[3801:5538,]
table(loans2_test$default)
table(loans2_train$default)

loans.m2<-glm(formula = default ~., data = loans2_train, family = 'binomial')
summary(loans.m2)

logistic_prob<-predict(loans.m2,loans2_test)
logistic_prediction<-ifelse(logistic_prob>.28951,1,0)
confusionMatrix(logistic_prediction,loans2_test$default) #Put this in the Presentaiton
```
...
loans_bi<-as.data.frame(model.matrix(~.-1,data=loans2))
loans_bi$default<-factor(loans_bi$default)
set.seed(12345)
loans_bi_rand<- loans_bi[order(runif(5538)), ]
loans_bi_train<-loans_bi_rand[1:3800,]
loans_bi_test<-loans_bi_rand[3801:5538,]
loans_bi_train_labels<-loans_bi_rand[1:3800,1]
loans_bi_test_labels<-loans_bi_rand[3801:5538,1]
table(loans_bi_test$default) 
table(loans_bi_train$default) 
loans_bi_knn<-knn(train = loans_bi_train,test = loans_bi_test, cl=loans_bi_train_labels)
CrossTable(x=loans_bi_test_labels,y=loans_bi_knn, prop.chisq = F)
confusionMatrix(loans_bi_knn,loans_bi_test$default, positive = '1')
...
loansDT1<-train(default ~ ., data = loans_bi_train, method='nnet' )
